...
@section('title', 'SMART - TAMBAH DATA INSTANSI DAN JABATAN')
@section('page_header', 'Tambah Data Instansi dan Jabatan')
@section('konten')
    <div class="section">
        <div class="card">
            <div class="card-body">
                <form action="{{ route('data.ixj.store') }}" method="post" class="form">
                    @csrf
                    <div class="row">
                        <div class="col-md-6 col-12">
                            ...
                            <select name="instansi" id="instansi" class="form-select select2" required>
                                ...
                                @foreach ($instansi as $item)
                                    <option value="{{ $item }}" {{ old('instansi') == $item ? 'selected' : '' }}>
                                        {{ $item }}
                                    </option>
                                @endforeach
