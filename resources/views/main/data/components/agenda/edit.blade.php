...
@section('konten')
    <div class="section">
        <div class="card">
            <div class="card-body">
                <form action="{{ route('data.agenda.update', $data->id) }}" method="post" class="form" data-parsley-validate id="formAgenda">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-6 col-12">
                            <div class="form-group">
                                ...
                                <input type="text" name="nama" id="nama" class="form-control"
                                    placeholder="Masukkan Nama Rapat" value="{{ old('nama', $data->nama) }}" required
                                    autofocus>
                            </div>
                        </div>
                        <div class="col-md-6 col-12">
                            <div class="form-group">
                                ...
                                <input type="date" name="tanggal" id="tanggal" class="form-control"
                                    value="{{ old('tanggal', $data->tanggal->format('Y-m-d')) }}" required>
                            </div>
                        </div>
                        <div class="col-md-6 col-12">
                            <div class="form-group">
                                ...
                                <input type="time" name="waktu_mulai" id="waktu_mulai" class="form-control"
                                    value="{{ old('waktu_mulai', isset($data) ? $data->waktu_mulai->format('H:i') : '') }}"
                                    required>
                            </div>
                        </div>
                        <div class="col-md-6 col-12">
                            <div class="form-group">
                                ...
                                <input type="time" name="waktu_selesai" id="waktu_selesai" class="form-control"
                                    value="{{ old('waktu_selesai', isset($data) ? $data->waktu_selesai->format('H:i') : '') }}"
                                    required data-parsley-time-after="#waktu_mulai"
                        ...
                        <div class="col-md-6 col-12">
                            <div class="form-group">
                                ...
                                <select name="status" id="status" class="form-select" required>
                                    ...
                                    <option value="Tidak Aktif" {{ old('status', $data->status) == 'Tidak Aktif' ? 'selected' : '' }}>Tidak
                                        Aktif</option>
                            ...
                            <div class="row" id="daftarPeserta"
                                ...
                                @foreach ($peserta as $p)
                                    <div class="col-12 col-md-4 col-lg-3 mb-2 item-peserta">
                                        <div
                                            class="card h-100 border shadow-sm {{ in_array($p->id, $pesertaTerpilih) ? 'border-primary bg-light' : '' }}">
                                            <div class="card-body p-2 d-flex align-items-center">
                                                ...
                                                <div style="line-height: 1.2;">
                                                    <strong class="nama-peserta d-block text-truncate"
                                                        style="font-size: 0.9rem; max-width: 150px;">{{ $p->nama }}</strong>
                                                    <small class="jabatan-peserta text-muted d-block"
                                                        style="font-size: 0.75rem; max-width: 150px;">{{ $p->jxi->nama_jabatan }}</small>
